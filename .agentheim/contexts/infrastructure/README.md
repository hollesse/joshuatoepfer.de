...
## Purpose
Owns globally-true infrastructure concerns: GitHub Pages deployment, GitHub Actions
...
A migration to Netlify is on the books as a planned future option (ADR-0001), but
GitHub Pages is the current hosting reality per ADR-0004.
...
- **Joshua** — reviews and merges sync PRs; maintains pipeline and hosting configuration
- **GitHub Pages** — hosts and serves the built static site (current). A migration to
  Netlify remains a planned future option per ADR-0001; the held-back `netlify.toml`
  in the repo root is kept on purpose for that eventual move.

## Ubiquitous language
- **Build** — the Jekyll process that compiles source files into a static site ready for
  deployment
- **Deploy** — GitHub Pages publishing a new build to the live joshuatoepfer.de URL
  (via the `actions/deploy-pages` workflow in `.github/workflows/deploy.yml`)
...
- `DeployPublished` — GitHub Pages made a new build live
